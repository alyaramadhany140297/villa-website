...
// Facility carousel - KODE FIX BATAS GESER & AUTOPLAY LEBIH CEPAT
(function(){
    ...
    // Fungsi utama untuk mengatur posisi geser
    function update(){ 
    ...
    // Fungsi untuk maju atau mundur
    function go(dir){ 
        index = Math.min(Math.max(index + dir, 0), maxIdx()); 
        update(); 
    }
    ...
    function start(){ 
        stop(); 
        timer = setInterval(() => { 
            if (index >= maxIdx()) index = 0; // Loop kembali ke awal jika sudah di akhir
            else go(1); 
        }, 2000); // <-- DIUBAH menjadi 2000 milidetik (2 detik)
    }
    
    // Fungsi Stop Autoplay
    function stop(){ if (timer){ clearInterval(timer); timer = null; } }
    
    // Fungsi Jeda Otomatis setelah interaksi manual
    ...
    // Perbarui tampilan saat ukuran layar berubah
    window.addEventListener("resize", () => {
        // Reset index ke 0 saat terjadi resize
        index = 0;
        update();
        pauseThenResume();
    });
