                ...
                <tbody>
                    ...
                    @forelse ($reviews as $review)
                        <tr>
                            ...
                            <td>{{ Str::limit($review->comment, 50) }}</td> <!-- Batasi ulasan agar tidak terlalu panjang -->
                            <td>
                                <!-- Menampilkan Bintang -->
                                @for ($i = 1; $i <= 5; $i++)
                                    <span class="star-icon" style="color: {{ $i <= $review->rating ? '#ffc107' : '#e0e0e0' }};">★</span>
                                @endfor
                                ({{ $review->rating }}/5)
                            </td>
                            <td>
                                ...
                                <span class="badge {{ $review->is_approved ? 'badge-success' : 'badge-warning' }}">
                                    {{ $review->is_approved ? 'Disetujui' : 'Belum Disetujui' }}
                                </span>
                            </td>
                            <td class="action-buttons">
                                ...
                                <form action="{{ route('admin.reviews.destroy', $review) }}" method="POST" style="display: inline-block;">
                                    @csrf
                                    <!-- Laravel tidak mendukung method PUT di form HTML, jadi kita gunakan @method('PUT') -->
                                    @method('PUT') 
                                ...
                                <form action="{{ route('admin.reviews.destroy', $review) }}" method="POST" style="display: inline-block;">
                                    @csrf
                                    <!-- Laravel tidak mendukung method DELETE di form HTML, jadi kita gunakan @method('DELETE') -->
                                    @method('DELETE')
